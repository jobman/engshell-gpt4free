...
openai.api_key = "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"
MAX_PROMPT = 4096
...
def LLM(prompt, mode='text'):
    ...
    """
    response = you.Completion.create(prompt=messages, chat=[])
    while "Please try again." in response.text.encode().decode("unicode_escape"):
        time.sleep(1.0/API_CALLS_PER_MIN)
        response = you.Completion.create(prompt=messages, chat=[])
    response_content = response.text.encode().decode("unicode_escape")
